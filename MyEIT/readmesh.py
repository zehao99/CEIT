...
class readmesh(object):
    """
    ONLY USE THIS WHEN USING NEW MESHES
    Read EITmesh.fem file
    And assign electrode centers and radius
    Change the electrode shape according to the mesh
    """
    def __init__(self, filename, electrode_centers, electrode_radius,folder_name = "", optimize_node_num = False, shuffle_element = False):
        self.nodes = []
        self.elements = []
        ...
        with open(self.folder_name + '/' + filename, newline = "") as file:
            for line in file:
                if line.find("GRID") != -1 and line.find("$") == -1:
                    x = self.stringtofloat(line[24:32].rstrip())
                    y = self.stringtofloat(line[32:40].rstrip())
    ...
    def stringtofloat(self, input_num):
        """
        parse data
        """
    ...
    def optimize_node_number(self):
        ...
        for i in range(steps + 1):
            ...
            for j, polar in enumerate(polar_list):
                if polar[0] <= (p_max - i * p_inc) and polar[0] > (p_max - (i + 1) * p_inc):
...
class read_mesh_from_csv_mm(object):
    """
    ...
    def __init__(self, name = 'Mesh_Cache', mode = 'pkl'):
        ...
        self.electrode_centers = np.array([[47,0],[47,23.5],[47,47],[23.5,47],\
            [0,47],[-23.5,47],[-47,47],[-47,23.5],\
                [-47,0],[-47,-23.5],[-47,-47],[-23.5,-47],\
                    [0,-47],[23.5,-47],[47,-47],[47,-23.5]])/1000
        self.electrode_num = len(self.electrode_centers)
...
def init_mesh(filename, electrode_centers, electrode_radius,folder_name = "", optimize_node_num = False,shuffle_element = False, draw = False):
    '''
