...
class EFEM(object):
    ...
    Parameters:
                mesh: dict with "node", "element" "perm"
                        ...
                        "perm": N_elem NDArray
                            contains permitivity on each element Default is 1
    ...
    def __init__(self, mesh, electrode_nums, electrode_center_list, electrode_radius, freqency = 20000.0 * 2 * np.pi ,perm = 1):
    ...
    def calculation(self,electrode_input):
        ...
        Return: node_potential, element_potential, electrode_potential 
        """
    ...
    def construct_sparse_matrix(self):
        ...
        for element in self.elem:
            ...
            for i,j in patern:
                if i != j :
                     # stiffness k_ij = sigma * (bk1*bk2 + ck1*ck2)/(4 * area) - j * w * capacitance * (bk1 * ck2 - bk2 * ck1) /24
                    K_ij = self.elem_perm[index] * (param[1+i] * param[1+j] + param[4+i] * param[4+j]) * (1 * param[0]) -  (self.freq * self.elem_capacitance[index] * param[0] /12) * 1j
                    self.K_sparse[element[i]][element[j]] += K_ij
                    self.K_sparse[element[j]][element[i]] += K_ij
                else:
                    K_ij = self.elem_perm[index] * (param[1+i] * param[1+j] + param[4+i] * param[4+j]) * (1 * param[0]) - (self.freq * self.elem_capacitance[index] * param[0] /6) * 1j
                    self.K_sparse[element[i]][element[j]] += K_ij
                    self.K_sparse[element[j]][element[i]] += K_ij
                
            index += 1
        #plt.imshow(np.abs(self.K_sparse))
        #plt.show
    ...
    def calculate_FEM(self, theta):
        ...
        Forward Problem
        """
        #print('Run task %s (%s)...' % (theta, os.getpid()))
    ...
    def calc_electrode_elements(self, electrode_number, center, radius):
        ...
        Parameters: 
                    electrode_number: INT current eletrode number
    ...
    def plot_current_capacitance(self):
        x, y = self.nodes[:, 0], self.nodes[:, 1]
        fig, ax = plt.subplots(figsize=(3.2, 3.2))
        im = ax.tripcolor(x, y, elem, np.abs(self.elem_capacitance), shading='flat', cmap='plasma')
        ax.set_aspect('equal')
    ...
    def plot_potential_map(self):
        x, y = self.nodes[:, 0], self.nodes[:, 1]
        fig, ax = plt.subplots(figsize=(3.2, 3.2))
        im = ax.tripcolor(x, y, elem, np.abs(self.element_potential), shading='flat', cmap='plasma')
        ax.set_aspect('equal')
        radius = self.electrode_radius
        for i, electrode_center in enumerate(self.electrode_center_list):
            x0 = electrode_center[0] - radius
