...
def save_parameter(param, filename, path_name="."):
    ...
    """
    with open(os.path.join(path_name ,'cache_' + filename + '.pkl'), "wb") as file:
...
def read_parameter(filename, path_name):
    ...
    """
    with open(os.path.join(path_name, 'cache_' + filename + '.pkl'), 'rb') as file:
...
def save_to_csv_file(data, filename, path_name="."):
    ...
    with open(os.path.join(path_name, filename), mode='w', newline='') as file:
...
def read_csv_from_file(filename, path_name="./"):
    ...
    data = []
    assert filename.endswith(".csv"), "The filename is not ended with csv."
    with open(os.path.join(path_name, filename), newline='') as file:
